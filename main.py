...
# Load environment variables from .env file (check app dir and parent)
from pathlib import Path
_load_paths = [
    Path(__file__).parent / ".env",           # COBALT-main/.env
    Path(__file__).parent.parent / ".env",     # Cobalt AI YEA/.env
]
...
def get_openrouter_client():
    ...
    if not api_key:
        return None
    # Use Railway/public URL when set, else localhost
    public_domain = os.environ.get("RAILWAY_PUBLIC_DOMAIN")
    referer = f"https://{public_domain}" if public_domain else os.environ.get("PUBLIC_URL", "http://localhost:5000")
    ...
    return OpenAI(
        ...
        default_headers={
            ...
            "X-Title": "COBALT AI Due Diligence",
        },
    )
...
if __name__ == "__main__":
    ...
    api_key_set = bool((os.environ.get("OPENROUTER_API_KEY") or "").strip())
    ...
    print(f"OPENROUTER_API_KEY: {'SET' if api_key_set else 'NOT SET (AI chat will not work)'}")
    print(f"=" * 50)
